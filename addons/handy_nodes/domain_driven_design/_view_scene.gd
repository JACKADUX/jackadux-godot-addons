...
var entity_view_mapper := DDD.EntityViewMapper.new()
...
func init_with_controller(p_controller:Object):
	...
	## init views
	# entity_view_mapper.init_views(controller.__get_entities())
	## update views
	# entity_view_mapper.update_views(controller.__get_entities())
	# entity_view_mapper.update_view(controller.__get_entity())
	
func bind_with_controller():
	# button.pressed.connect(func(): app_controller.do_something())
	entity_view_mapper.init_fns(view_container, 
		func new_view_fn(): 
			return Button.new()
			,
		func init_view_fn(view:Button): 
			view.pressed.connect(func():
				# WARNING: 如果需要处理交互反馈，这里必须要用 entity_view_mapper.get_entity_data() 方法获取最新 entity_data
				var entity_data = entity_view_mapper.get_entity_data(view)
